...
async function fetchShippingData(shippingId: string | number, token: string) {
  try {
    ...
    // 🔥 FIX: Solo descuentos del SENDER (vendedor).
    // receiver.discounts = descuento de envío dado al COMPRADOR (ej: envío gratis para el buyer).
    // senders.discounts = bonificación que ML le paga al VENDEDOR.
    // Antes se sumaban ambos → doble conteo (ej: $6.490 + $6.490 = $12.760 incorrecto).
    const senderDiscounts = data?.senders?.[0]?.discounts ?? []
    const bonificacion = Array.isArray(senderDiscounts)
      ? senderDiscounts.reduce((acc: number, d: any) => acc + Number(d.promoted_amount ?? 0), 0)
      : 0
